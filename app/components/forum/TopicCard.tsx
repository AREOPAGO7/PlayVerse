...
export default function TopicCard({ topic, onClick }: { topic: ForumTopic; onClick: (id: string) => void }) {
    return (
        <div 
            ...
            className="bg-zinc-800/50 rounded-lg p-4 md:p-6 hover:bg-zinc-800 transition-colors cursor-pointer"
        >
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-3">
                        <div className="w-8 h-8 rounded-full overflow-hidden flex-shrink-0">
                            <img src={topic.userAvatar} alt={topic.username} className="w-full h-full object-cover" />
                        </div>
                        <span className="text-zinc-400 truncate">{topic.username}</span>
                        <span className="text-zinc-500 text-[12px] ml-auto flex-shrink-0">
                            {formatDistanceToNow(new Date(topic.createdAt.seconds * 1000), { addSuffix: true })}
                        </span>
                    </div>
                    
                    <div className="mb-4">
                        <h3 className="text-lg font-semibold text-white mb-2 line-clamp-2">{topic.title}</h3>
                        <p className="text-zinc-400 text-base leading-relaxed line-clamp-3">{topic.description}</p>
                    </div>
