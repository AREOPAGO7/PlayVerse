...
const Browse = () => {
    ...
    const [openDropdown1, setOpenDropdown1] = useState<number | null>(null);
                                ...
                                {currentGames.map((game) => (
                                    ...
                                    >
                                        <div className={`relative overflow-hidden rounded-lg ${!gridView && "w-16"}`}>
                                            <Image src={game.image || "/placeholder.svg"} alt={game.title} width={200} height={300}
                                        ...
                                        <div className={gridView ? "mt-2" : "flex-1"}>
                                            <div className="flex justify-between items-start">
                                                ...
                                                <button
                                                    onClick={() => setOpenDropdown(openDropdown === game.id ? null : game.id)} className="h-6 w-6 pt-3 flex items-center justify-center hover:bg-gray-700 rounded-full relative">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"  stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                        ...
                                                        <div className="absolute right-0 left-6 top-full mt-1 w-48 rounded-xl bg-zinc-800 shadow-lg z-50">
                                                            <div className="py-1">
                                                                ...
                                                                <div className="border-b border-white/10 mx-auto w-[90%]">
                                                                    <div onClick={() => {/* Add install handler */ }}
                                                                        className="flex items-center px-4 rounded-lg py-2 text-sm text-gray-300 hover:bg-zinc-600 cursor-pointer">
                                                                        <svg className="mr-3 h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        ...
                                        {Array.from({ length: Math.ceil(games.length / itemsPerPage) }, (_, i) => (
                                            <li key={i}>
                                                <button
                                                    ...
                                                    className={`px-3 py-1 mx-1 rounded ${currentPage === i + 1 ? 'bg-zinc-800 text-white' : 'bg-zinc-700 text-gray-300 hover:bg-zinc-600'}`}
                                                >
                                                    {i + 1}
                                                </button>
                                            </li>
                    ...
                    <div className="w-full ml-6 lg:w-60 space-y-4">
                        ...
                        {Object.keys(openFilters).map((filter) => (
                            <div key={filter} className="border-b border-gray-800 pb-2">
                                ...
                                >
                                    {filter}
                                    <svg  xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`transition-transform duration-200 ${openFilters[filter] ? "rotate-180" : ""}`}>
                                        <path d="m6 9 6 6 6-6" />
                                    </svg>
                                </button>
                                {openFilters[filter] && (
